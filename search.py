...
def add_to_queue(nodes_queue, nodes_expanded, encountered_nodes, nodes_not_seen):
    unseen_nodes = []
    ...
    if len(nodes_expanded) > 0 and nodes_expanded[0][2] == 0:   # If not UUCS, it should have been goal state
        while len(nodes_expanded) > 0:
            nodes_queue.append(nodes_expanded.pop(0))           # Put first element in queue

    else:
        while len(nodes_expanded) > 0:
            added = False
            tmp = nodes_expanded.pop()
    ...
    return nodes_queue, nodes_expanded, encountered_nodes, nodes_not_seen


#   DONT FORGET TO CHECK IF WE"VE SEEN THE NODE BEFORE OR NOT. DO THIS IN THE WHILE LOOP
def search(node, heuristic):
    nodes = []                  # Make queue
    ...
    nodes, expanded_nodes, encountered_nodes, nodes_not_seen = add_to_queue(nodes,
                            expanded_nodes, encountered_nodes, nodes_not_seen)
    ...
    while len(nodes) > 0:
        ...
        #for i in range(0, len(expanded_nodes)):
        #    print(expanded_nodes[i][0])
        expanded_nodes = apply_depth_and_heuristic(expanded_nodes, heuristic)
        add_to_queue(nodes, expanded_nodes, encountered_nodes, nodes_not_seen)
